...
const { ERR, MS_CLIENT_ID, MS_CLIENT_SECRET, MS_REDIRECT } = require("../common/constants.js");
...
async function handleRouteAuthMSCallback(req, res) {
  ...
  const accessRes = await fetch("https://login.microsoftonline.com/common/oauth2/v2.0/token", {
    method: "POST",
    headers: {
  ...
  await USER_DB.pushUser(new User(newUid, username, email, generatePassword(16)));
